export const meta = {
  name: 'gauntlet-wave11-critics',
  description: 'Gauntlet run 3 wave 11: five blind cross-model critics plus one zero-builder smoothing pass',
  phases: [{ title: 'Judge', detail: 'five blind critics on a non-Anthropic family, one smoothing pass' }],
}

const CAP = '.gauntlet/waves/core-after-docks/11'
...
// BYTE-IDENTICAL to waves 4-10's CRITIC_SHARED, copied from wave10-critics.mjs.
...
const SMOOTHING = `You are the SMOOTHING PASS for gauntlet run 3, wave 11, on the claude-wrapper
  ...
  .gauntlet/waves/core-after-docks/10/critics-report.md.

  ## THE CAPTURES

  Wave 11 (current): ${CAP}/
  Wave 10 (previous): .gauntlet/waves/core-after-docks/10/
  Waves 1-9: .gauntlet/waves/core-after-docks/<N>/

  The twelve wave-11 files hashed IDENTICAL to wave 10 before you were launched. This is the THIRD
  ...
  Every wave-10 critic gap was already-landed, refused, or owner/product-shaped:

  - Welcome: headline as the ~412px column measure restates 3.3 / 9.4 (one-line display is authored).
  - Titlebar: the asked 16px name-to-pill break already paints (9 / 16 / 4); shrinking the pills is owner-shaped.
  - Sidebar: collapsing five pre-list bands to ~100px is the wave 3/4 compression axis; wave 4 already reverted a fold on a fence.
  - Chat: hiding SHOW rows at rest is the same ToolCard JSX/state change as 9.7, not CSS.
  - InputBar: a left-aligned 12-16px Effort/Model cluster is the eleventh distribution rearrangement, refused under 5.8.
  ...
  measure whether the artifact stayed stable and whether the existing coherence seams remain real.
    If this wave also moves no verdict up, plateau goes 2 -> 3 and the run stops. Do not invent a
    last-second piece to keep the chain alive.

  ## CONTROLS TO RE-MEASURE

  1. **NULL CONTROL.** Compare every wave-11 capture with wave 10 by bytes and RGB pixels. Expected:
  ...
  Does a third consecutive zero-pixel wave reveal any NEW cross-surface seam that is independently
...
const SMOOTHING_SCHEMA = {
  ...
  properties: {
    ...
    nullControl: { type: 'string', description: 'All 12 wave-11 captures against wave 10 by bytes and RGB pixels' },
    ...
    markControl: { type: 'string', description: 'Marks byte-identical to wave 10 at all sites' },
