export const meta = {
  name: 'gauntlet-wave9-critics',
  description: 'Gauntlet run 3 wave 9: five blind cross-model critics plus one zero-builder smoothing pass',
  phases: [{ title: 'Judge', detail: 'five blind critics on a non-Anthropic family, one smoothing pass' }],
}

const CAP = '.gauntlet/waves/core-after-docks/9'
...
// BYTE-IDENTICAL to waves 4-8's CRITIC_SHARED, copied from wave8-critics.mjs and
// hash-verified against f89141c58449127c before launch. Verdicts are only comparable
// across waves if the instrument does not drift.
//
...
const SMOOTHING = `You are the SMOOTHING PASS for gauntlet run 3, wave 9, on the claude-wrapper
...
.gauntlet/waves/core-after-docks/8/critics-report.md.

## THE CAPTURES

Wave 9 (current): ${CAP}/
Wave 8 (previous): .gauntlet/waves/core-after-docks/8/
Waves 1-7: .gauntlet/waves/core-after-docks/<N>/

All twelve wave-9 files are byte-identical to wave 8. This is a ZERO-BUILDER measurement wave.
Do not invent movement. Prove the null control yourself rather than trusting this briefing. The
twelfth file, window-session-short.png, is yours but is withheld from every critic.

## WHY ZERO BUILDERS

Every wave-8 critic gap was blocked, refuted, product-shaped, or owner-shaped:

- Welcome: a recent-project action plane adds a new data/action contract.
- Titlebar: relocating both pills beside the centered workspace title is an owner decision after
  six waves tuned the left lockup.
- Sidebar: adding 16-20px vertically undoes the landed compression and moves the y202 fence.
- Chat: the alleged missing top inset is an autoscroll artifact; source ships 24px and the first
  visible rounded corner is whole.
- InputBar: the compact-cluster ask is the ninth distribution rearrangement across three runs.
...
1. **NULL CONTROL.** Compare every wave-9 capture with wave 8 by bytes and RGB pixels. Expected:
...
Does a zero-pixel wave reveal any NEW cross-surface seam that is independently measurable and not
one of the eight settled/refused axes above? If not, return no new piece. Do not turn a stationary
artifact into speculative work.
...
const SMOOTHING_SCHEMA = {
  ...
  properties: {
    ...
    nullControl: { type: 'string', description: 'All 12 wave-9 captures against wave 8 by bytes and RGB pixels' },
    ...
    markControl: { type: 'string', description: 'Marks byte-identical to wave 8 at all sites' },
