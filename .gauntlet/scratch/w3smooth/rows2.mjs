...
function ins(d,c){const W=d.w,a=[]
  for(let dy=0;dy<Math.min(c.y1-c.y0+1,22);dy++){let x=c.x0;while(x<=c.x1&&!c.px.has((c.y0+dy)*W+x))x++;a.push(x-c.x0)}
  let k=0;while(k<a.length&&a[k]>0)k++;return {a:a.slice(0,Math.max(k+2,5)),k}}
for(const surf of ['agents-dock','sidebar','commands-dock']){
  ...
  console.log(`\n### ${surf}`)
  for(const [k,v] of [...m.entries()].sort((a,b)=>b[1]-a[1]).slice(0,10)){
    if(v<400)continue
    const rgba=k.split(',').map(Number)
    for(const c of comps(d,rgba,400)){
      const w=c.x1-c.x0+1,h=c.y1-c.y0+1
      const fill=c.n/(w*h)
      if(w<40||h<12||h>200||fill<0.55||w>246)continue
      const r=ins(d,c)
      console.log(`  ${k.padEnd(18)} ${String(w).padStart(4)}x${String(h).padStart(3)} at x${c.x0}..${c.x1} y${c.y0}..${c.y1} fill=${(100*fill).toFixed(0)}% r~${r.a[0]} rRows=${r.k} ratio=${(r.a[0]/h).toFixed(3)} prof=[${r.a.join(',')}]`)
    }
  }
}
