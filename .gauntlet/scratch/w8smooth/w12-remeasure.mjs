...
console.log('\n===== 3 TYPE SCALE =====')
{
  ...
  const GR = [11, 15, 17]
  {
    const I = decode(`${W12}/titlebar.png`)
...
{
  const S = decode(`${W12}/sidebar.png`)
  const C = decode(`${W12}/chat.png`)
  ...
  const K = decode(`${W12}/commands-dock.png`)
...
{
  const S = decode(`${W12}/sidebar.png`)
  const T = decode(`${W12}/titlebar.png`)
...
console.log('\n===== 8 DATE DIVIDER =====')
{
  const I = decode(`${W12}/window-session-short.png`)
  const BD = [3, 6, 6]
  const on = (x, y, thr = 2) => {
    const [r, g, b] = I.at(x, y)
...
console.log('\n===== 9 JOG =====')
{
  ...
  const jobs = [
    ['SHORT', `${W12}/window-session-short.png`, null, null],
    ['OVERFLOW', `${W12}/window-session.png`, [60, 760], [775, 899]]
  ]
...
{
  const cmp = (f, box) => {
    const A = decode(`${W11}/${f}`)
    const B = decode(`${W12}/${f}`)
  ...
  const T = decode(`${W12}/titlebar.png`)
  const W = decode(`${W12}/welcome.png`)
  const C = decode(`${W12}/chat.png`)
