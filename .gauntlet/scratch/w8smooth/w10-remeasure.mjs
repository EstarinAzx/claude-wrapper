...
console.log('===== 1 NULL CONTROL wave 10 vs wave 9 =====')
{
  let byteIdentical = 0
  ...
  for (const file of FILES) {
    ...
    const sha = createHash('sha256').update(b10).digest('hex').slice(0, 16)
...
console.log('\n===== 2 IDENTITY FLOOR wave 10 =====')
{
  const hues = new Map()
  let worst = { f: '', share: 0, mint: 0, tot: 0 }
  for (const f of FILES) {
    const I = decode(`${W10}/${f}`)
    let mint = 0
    const tot = I.w * I.h
    const local = new Map()
...
console.log('\n===== 3 TYPE SCALE =====')
{
  ...
  const GR = [11, 15, 17]
  {
    const I = decode(`${W10}/titlebar.png`)
...
{
  const S = decode(`${W10}/sidebar.png`)
  const C = decode(`${W10}/chat.png`)
  ...
  const K = decode(`${W10}/commands-dock.png`)
...
{
  const S = decode(`${W10}/sidebar.png`)
  const T = decode(`${W10}/titlebar.png`)
...
console.log('\n===== 8 DATE DIVIDER =====')
{
  const I = decode(`${W10}/window-session-short.png`)
  const BD = [3, 6, 6]
  const on = (x, y, thr = 2) => {
    const [r, g, b] = I.at(x, y)
...
console.log('\n===== 9 JOG =====')
{
  ...
  const jobs = [
    ['SHORT', `${W10}/window-session-short.png`, null, null],
    ['OVERFLOW', `${W10}/window-session.png`, [60, 760], [775, 899]]
  ]
...
console.log('\n===== 10 MARKS vs wave 9 =====')
{
  ...
  const T = decode(`${W10}/titlebar.png`)
  const W = decode(`${W10}/welcome.png`)
  const C = decode(`${W10}/chat.png`)
