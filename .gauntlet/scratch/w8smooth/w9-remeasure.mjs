...
console.log('===== 2 IDENTITY FLOOR wave 9 =====')
const hues = new Map()
let worst = { f: '', share: 0 }
...
console.log(`  WORST ${worst.f} ${f2(worst.share)}%`)
...
console.log('\n===== 4 TITLEBAR =====')
{
  const I = decode(`${ROOT}/titlebar.png`)
  ...
  console.log(`  intervals ${intervals.join('/')} ratio ${f2(intervals[1] / intervals[0])}x`)
  // session title
  const occ2 = []
  for (let x = 0; x < I.w; x++) { let n = 0; for (let y = 1; y <= 45; y++) if (on(x, y)) n++; occ2.push(n) }
  const runs2 = []; let cur2 = null
  for (let x = 0; x < I.w; x++) { if (occ2[x] > 0) { if (!cur2) cur2 = { x0: x }; cur2.x1 = x } else if (cur2) { runs2.push(cur2); cur2 = null } }
...
console.log('\n===== 6 QUIET CONTROL SEAM =====')
{
  ...
  console.log(`  rail ground rgb(${railG}) L=${rL.toFixed(4)}`)
  console.log(`  field fill rgb(${fieldFill}) L=${fL.toFixed(4)} step ${(fL - rL >= 0 ? '+' : '') + (fL - rL).toFixed(4)}`)
  console.log(`  card surface rgb(${cardSurf}) L=${cL.toFixed(4)}`)
  console.log(`  row fill rgb(${rowFill}) L=${rfL.toFixed(4)} step ${(rfL - cL >= 0 ? '+' : '') + (rfL - cL).toFixed(4)}`)
  console.log(`  row border rgb(${rowBorder}) L=${rbL.toFixed(4)} step ${(rbL - cL >= 0 ? '+' : '') + (rbL - cL).toFixed(4)}`)
  // field left edge
  let firstCol = -1
  for (let x = 0; x < S.w; x++) if (near(S.at(x, 130), fieldFill, 3)) { firstCol = x; break }
  console.log(`  field mid left x${firstCol}`)
  // field straight-run at x16
  let run = 0, best = 0
...
console.log('\n===== 8 DATE DIVIDER =====')
{
  ...
  console.log(`  ink block y${t}..${btm} clear above ${t - 48} below ${n0 - btm - 1}`)
  // box clearance: 18px line box y88..105
  console.log(`  box y88..105 vs last-above/next: last ink above divider-block, 40px authored`)
}
...
console.log('\n===== 9 JOG =====')
{
  ...
  const jobs = [
    ['SHORT', `${ROOT}/window-session-short.png`, null, null],
    ['OVERFLOW', `${ROOT}/window-session.png`, [60, 760], [775, 899]]
  ]
...
{
  const cmp = (f, box) => {
    const A = decode(`D:/.claude/claude projects/playground/4/.gauntlet/waves/core-after-docks/8/${f}`)
    const B = decode(`${ROOT}/${f}`)
...
console.log('\n===== CANDIDATE: dock row shells =====')
{
  ...
  for (const file of files) {
    ...
    console.log(`  ${file} tint-2-ish rgb(25,30,32) comps>=20: ${components.length}`)
    for (const c of components.slice(0, 8)) console.log(`    x${c.x0}..${c.x1} y${c.y0}..${c.y1} ${c.w}x${c.h} n=${c.n}`)
  }
}

console.log('\n===== CANDIDATE: appearance vs command vs agent row fill/outline =====')
{
  const sample = (file, pts) => {
    const I = decode(`${ROOT}/${file}`)
    for (const [lbl, x, y] of pts) {
      const c = I.at(x, y)
      const o = oklch(...c)
      console.log(`  ${file} ${lbl} @${x},${y} rgb(${c}) L=${o.L.toFixed(4)} C=${o.C.toFixed(4)}`)
    }
  }
  sample('commands-dock.png', [
    ['dock ground', 120, 30],
    ['row interior', 120, 80],
    ['row edge mid', 7, 80],
    ['row edge right', 241, 80]
  ])
  sample('appearance-dock.png', [
    ['dock ground', 120, 80],
    ['theme selected interior', 120, 80],
    ['ember interior', 120, 130],
    ['acrylic interior', 120, 370],
    ['mica interior', 120, 450]
  ])
  sample('agents-dock.png', [
    ['dock ground', 120, 80],
    ['row interior', 120, 80],
    ['nested interior', 80, 180]
  ])
  sample('sidebar.png', [
    ['rail ground', 120, 320],
    ['active row', 120, 240],
    ['inactive row', 120, 320]
  ])
}

console.log('\n===== CANDIDATE: close-mark identity across docks =====')
{
  const files = ['agents-dock.png', 'commands-dock.png', 'appearance-dock.png']
  const boxes = []
  for (const f of files) {
    const I = decode(`${ROOT}/${f}`)
    // close sits in the 44px head, right side
    let x0 = 1e9, x1 = -1, y0 = 1e9, y1 = -1, n = 0
    for (let y = 8; y <= 36; y++) for (let x = 200; x <= 247; x++) {
      const [r, g, b] = I.at(x, y)
      if (r + g + b > 80) { n++; if (x < x0) x0 = x; if (x > x1) x1 = x; if (y < y0) y0 = y; if (y > y1) y1 = y }
    }
    console.log(`  ${f} head-right ink x${x0}..${x1} y${y0}..${y1} w${x1 - x0 + 1} h${y1 - y0 + 1} n${n}`)
    boxes.push({ f, I, x0, y0, x1, y1 })
  }
}

console.log('\n===== CANDIDATE: titlebar vs welcome mark size =====')
{
  const T = decode(`${ROOT}/titlebar.png`)
  ...
  console.log('  chat avatar1', bbox(C, 190, 260, 90, 150))
  console.log('  chat avatar2', bbox(C, 190, 260, 640, 700))
}

console.log('\n===== CANDIDATE: composer Default chips vs titlebar pills =====')
{
  const B = decode(`${ROOT}/input-bar.png`)
  const T = decode(`${ROOT}/titlebar.png`)
  const sampleBox = (I, x0, x1, y0, y1, lbl) => {
    const m = new Map()
    for (let y = y0; y <= y1; y++) for (let x = x0; x <= x1; x++) {
      const k = I.at(x, y).join(',')
      m.set(k, (m.get(k) || 0) + 1)
    ...
    console.log(`  ${lbl} top ${top.map(([k, n]) => `${k}:${n}`).join(' ')}`)
  }
  sampleBox(T, 158, 216, 13, 33, 'titlebar Wisped pill')
  sampleBox(T, 221, 275, 13, 33, 'titlebar Bypass pill')
  // input-bar chips: Default chips sit under the pill
  sampleBox(B, 300, 400, 70, 100, 'input-bar mid-lower')
}

console.log('\n===== CANDIDATE: head heights =====')
{
  const files = [
    ['sidebar.png', 248],
    ['agents-dock.png', 248],
    ['commands-dock.png', 248],
    ['appearance-dock.png', 248]
  ]
  for (const [f, w] of files) {
    ...
    for (let y = 30; y <= 50; y++) {
      let n = 0
      for (let x = 0; x < w; x++) if (near(I.at(x, y), hair, 4)) n++
      if (n >= w - 8) { hairY = y; break }
    }
    console.log(`  ${f} first full hairline y${hairY} ground rgb(${G})`)
  }
}
