...
pieces:
  # Wave 8: held BAR WINS, change SAME (pixel-identical to wave 7). Critic
  ...
  # Wave 10: held BAR WINS, change SAME. Critic-only / collision A.
  # Wave 11: held BAR WINS, change SAME (pixel-identical to wave 10). Critic-only.
  # Gap restates left-edge / Explore compact child row — still collision A.
  # Wave 12: held BAR WINS, change SAME (pixel-identical to wave 11). Critic-only.
  # Gap: shorten Explore branch rule to child content height + tighten desc-to-meta.
  # Still collision A — do not build shells.
  - name: AgentsDock
    verdict: BAR WINS
    open: true
  ...
  # Wave 8: held BAR WINS. Gap 2 -> 4 built and verified; change BETTER inside
  ...
  # Wave 10: BAR WINS -> YOURS WINS. Zoom head got appearance-field--control
  # resting --tint-2 shell binding mark+label+compact stepper. CLOSED.
  # Critic's next gap (Theme selected-descriptor) is post-close polish — do not
  # reopen by default; Theme rows stay swatch-only by design (DESIGN.md / copy).
  - name: AppearanceDock
    verdict: YOURS WINS
    open: false
  ...
  # Wave 10: held BAR WINS, change BETTER inside band (Appearance Zoom). Still A.
  # Wave 11: held BAR WINS, change SAME (pixel-identical). Critic-only. Still A.
  # Wave 12: held BAR WINS, change SAME (pixel-identical). Critic-only. Still A.
  # Gap: top-level agent spacing + nested Explore subordinate via type/indent alone.
  - name: DocksAsOne
    verdict: BAR WINS
    open: true
...
| 8 | AgentsDock | BAR WINS | *(no builder — collision A / owner call 19; capture pixel-identical to wave 7)* Replace each loose agent text stack with a full-width, consistently padded row shell that aligns name, description, metadata, and selection or state affordances while preserving child indentation within the parent hierarchy. **SPEC BREAK refused — see wave 8 adjudication 2.** |
| 8 | CommandsDock | YOURS WINS | *(closed — no critic this wave; capture pixel-identical)* |
| 8 | AppearanceDock | BAR WINS | *(held — change BETTER inside the same ordinal; gap 2 -> 4 closed the corrugated read)* Remove the always-on rounded shells from the three inactive Theme rows, reserving the shell for the selected option so the group reads as a choice list rather than a stack of empty fields. **READ ADJUDICATION 3 — this inverts wave 7's resting-all-shells move and collides with Commands + DocksAsOne.** |
...
| 10 | AgentsDock | BAR WINS | *(no builder — collision A / owner call 19; capture pixel-identical to wave 9)* Align each nesting level to one shared left edge and tighten the vertical rhythm around Explore so its title, description, and metadata read as one deliberate child row rather than a text stack hanging from a rule. **Still collision A if read as resting shells — do not build shells without owner call 19.** |
| 10 | CommandsDock | YOURS WINS | *(closed — no critic this wave; capture pixel-identical)* |
| 10 | AppearanceDock | **YOURS WINS** | **THE VERDICT MOVED.** *(builder — Zoom head `appearance-field--control` resting --tint-2 shell; mark+label+compact stepper one composed row; Theme/Backdrop bare; option shells + gap-4 held)* Give the Theme section one concise selected-theme descriptor or preview so its four compact swatch rows feel as authored as the Backdrop choices. **Post-close polish — Theme is swatch-only by design; do not reopen by default.** |
| 10 | WelcomeMinWindow | YOURS WINS | *(closed — no critic this wave; capture pixel-identical; headroom still 69.71/70)* |
| 10 | DocksAsOne | BAR WINS | *(held — change BETTER from Appearance's Zoom-row compose; Agents/Commands pixel-identical)* Tighten Agents with consistent spacing between top-level entries and a more clearly subordinate type treatment for nested Explore, without adding resting shells. **Still collision A — do not build Agents shells without a human pick.** |
| 10 | IconHousing | YOURS WINS | *(closed — no critic this wave; capsule kept; collision B held)* |
| 11 | AgentsDock | BAR WINS | *(no builder — collision A / owner call 19; capture pixel-identical to wave 10)* Align each nesting level to one shared left edge and tighten the spacing around Explore so its title, wrapped description, and metadata read as one compact child row rather than a text stack hanging from an oversized vertical rule. **Still collision A if read as resting shells — do not build shells without owner call 19.** |
| 11 | CommandsDock | YOURS WINS | *(closed — no critic this wave; capture pixel-identical)* |
| 11 | AppearanceDock | YOURS WINS | *(closed — no critic this wave; capture pixel-identical; Theme-descriptor polish still not a reopen)* |
| 11 | WelcomeMinWindow | YOURS WINS | *(closed — no critic this wave; capture pixel-identical; headroom still 69.71/70)* |
| 11 | DocksAsOne | BAR WINS | *(held — change SAME; critic-only; all three dock captures pixel-identical to wave 10)* Agents still breaks the cross-dock finish: regularize spacing between top-level entries and make nested Explore visibly subordinate with smaller, quieter type and indentation alone. **Still collision A — do not build Agents shells without a human pick.** |
| 11 | IconHousing | YOURS WINS | *(closed — no critic this wave; capsule kept; collision B held)* |
| 12 | AgentsDock | BAR WINS | *(no builder — collision A / owner call 19; capture pixel-identical to wave 11)* Shorten the Explore branch rule to the child content height and tighten the description-to-metadata gap so the nested item reads as one compact unit rather than a loose text stack. **Still collision A if read as resting shells — do not build shells without owner call 19.** |
| 12 | CommandsDock | YOURS WINS | *(closed — no critic this wave; capture pixel-identical)* |
| 12 | AppearanceDock | YOURS WINS | *(closed — no critic this wave; capture pixel-identical; Theme-descriptor polish still not a reopen)* |
| 12 | WelcomeMinWindow | YOURS WINS | *(closed — no critic this wave; capture pixel-identical; headroom still 69.71/70)* |
| 12 | DocksAsOne | BAR WINS | *(held — change SAME; critic-only; all three dock captures pixel-identical to wave 11)* In Agents, increase the spacing between top-level agent entries and make the nested Explore block visibly subordinate through smaller type and stronger indentation alone. **Still collision A — do not build Agents shells without a human pick.** |
| 12 | IconHousing | YOURS WINS | *(closed — no critic this wave; capsule kept; collision B held)* |

## Wave 12 adjudications — critic-only stall, ordinals held, plateau 1 -> 2

**Zero builders, two open-piece critics, zero smoothing edits.** Gate green (D7); **zero `SPEC BREAK`s**; critic family re-resolved live to `sonnet` -> `codex/gpt-5.6-sol`; `critic_degraded: false`. Final CSS bundle **still `index-DOI17h8g.css`** (unchanged from waves 10–11 — no `src/` edit).
...
| AppearanceDock | YOURS WINS | YOURS WINS | closed | — |
...
On the preset's literal rule none improved, so **`plateau` 1 -> 2**. Halt still at `plateau: 3` — **one clean wave away** — OR at the next firing's stop check because **`wave` is now 12 (`max_waves`)**. Owner call 19 still open, still default build-nothing on Agents/DocksAsOne shells. Owner call 20 still open, still not acted on.

### 2. WAVE 12 WAS HONESTLY CRITIC-ONLY — NO CLEAN PRIMARY BUILD

Wave 11 left only Agents + DocksAsOne open, both still collision A / owner call 19. Default taken: capture + critic only. All eleven captures **SHA256-identical** to wave 11 (including null controls `titlebar.png` and `sidebar.png`). Critics both answered `change: SAME`, matching the bytes.

### 3. GAPS RESTATE THE SAME BLOCKED LEVER — STILL NO NON-SHELL BUILD WITHOUT OWNER CALL 19

- **AgentsDock:** shorten Explore branch rule to child content height + tighten description-to-metadata gap so nested item reads as one compact unit. Same family as waves 9–11 (Explore compact child). Still collides with resting-shell reads of owner call 19 if converted into shells.
- **DocksAsOne:** increase top-level agent entry spacing; make nested Explore subordinate via smaller type and stronger indentation **alone**. Same family as waves 10–11. Still collision A if converted to shells. Critic notes Commands and Appearance look finished; Agents still prevents the three docks reading as one equally resolved system.

No non-colliding, non-inverting lever appeared. Do not invent one off Appearance's post-close Theme-descriptor polish.

### 4. BOTH COLLISIONS HELD AT THE DEFAULT

**Collision A:** no builder on Agents, no builder on DocksAsOne. Default remains build nothing.
**Collision B:** IconHousing stays YOURS WINS with the capsule. Do not remove it.
...
- **Build before capture.** Bundle confirmed `index-DOI17h8g` (wave 10/11 hash) — correct for a no-edit wave. No `npm run build` required.
- **Inspect PASS**, headroom 69.71 / claimed 70, overflow 0, 11/11 files under `.gauntlet/waves/docks-and-min-window/12/`.
- **SHA256: all 11 SAME vs wave 11**, including null controls.
- **Drivers wave2..wave10 ALL GREEN** (exit 0 each). No wave12 driver (no build). Wave9 has no driver either.
- **Gate:** typecheck clean; **96 files / 1406 passed + 44 skipped**; no `src/` change.
- **Critic routing re-resolved live** this wave: `sonnet` -> `codex/gpt-5.6-sol`. Thirteen consecutive waves, each read fresh.

### 7. What the next firing inherits

| piece | next posture | trap |
|---|---|---|
| AgentsDock / DocksAsOne | **still collision A** — only open pieces | row shell; owner call 19 — default remains build nothing |
| AppearanceDock | closed YOURS | Theme descriptor is post-close polish — do not reopen; do not invert shells |
...
**Stop check on the NEXT firing:** `wave: 12` and `max_waves: 12` → **`wave >= max_waves` budget backstop fires before any wave 13.** Plateau is 2 (one shy of 3). Expected exit: budget halt with four YOURS WINS closed and Agents/DocksAsOne still BAR WINS behind owner call 19.

## Wave 11 adjudications — critic-only stall, ordinals held, plateau 0 -> 1

**Zero builders, two open-piece critics, zero smoothing edits.** Gate green (D7); **zero `SPEC BREAK`s**; critic family re-resolved live to `sonnet` -> `codex/gpt-5.6-sol`; `critic_degraded: false`. Final CSS bundle **still `index-DOI17h8g.css`** (unchanged from wave 10 — no `src/` edit).
...
On the preset's literal rule none improved, so **`plateau` 0 -> 1**. Halt still at `plateau: 3` — **two clean waves away**. Owner call 19 still open, still default build-nothing on Agents/DocksAsOne shells. Owner call 20 still open, still not acted on.

### 2. WAVE 11 WAS HONESTLY CRITIC-ONLY — NO CLEAN PRIMARY BUILD

Wave 10 closed Appearance and left only Agents + DocksAsOne open, both still collision A / owner call 19. Default taken: capture + critic only. All eleven captures **SHA256-identical** to wave 10 (including null controls `titlebar.png` and `sidebar.png`). Critics both answered `change: SAME`, matching the bytes. Agents critic confirmed same SHA-256 hash as wave 10.

### 3. GAPS RESTATE THE SAME BLOCKED LEVER — NO NEW NON-SHELL CANDIDATE

- **AgentsDock:** left-edge alignment + compact Explore child row (title/desc/meta as one unit). Same family as waves 9–10. Still collides with resting-shell reads of owner call 19.
- **DocksAsOne:** Agents top-level spacing + subordinate Explore type/indent, **without** resting shells. Same family as wave 10. Still collision A if converted to shells.
...
- **Build before capture.** Bundle confirmed `index-DOI17h8g` (wave 10's hash) — correct for a no-edit wave.
- **Inspect PASS**, headroom 69.71 / claimed 70, overflow 0, 11/11 files under `.gauntlet/waves/docks-and-min-window/11/`.
- **SHA256: all 11 SAME vs wave 10**, including null controls.
- **Drivers wave2..wave10 ALL GREEN.** No wave11 driver (no build). Wave4 timed out on its own TIMEOUT banner but exit 0 / ALL GREEN.
- **Gate:** typecheck clean; **96 files / 1406 passed + 44 skipped**; no `src/` change so no rebuild required beyond the confirmed bundle.
- **Critic routing re-resolved live** this wave: `sonnet` -> `codex/gpt-5.6-sol`. Twelve consecutive waves, each read fresh.

### 7. What wave 12 inherits

| piece | wave 12 posture | trap |
|---|---|---|
| AgentsDock / DocksAsOne | **still collision A** — only open pieces | row shell; owner call 19 — default remains build nothing; no new non-shell lever this wave |
| AppearanceDock | closed YOURS | Theme descriptor is post-close polish — do not reopen by default; do not invert shells |
| CommandsDock | closed YOURS | optional horizontal padding only if re-opened |
| WelcomeMinWindow | closed YOURS | preserve 24; do not tighten |
| IconHousing | closed YOURS | zoom-minus only if re-opened; **do not remove the capsule** |

With plateau at 1 and only Agents/DocksAsOne still open — both blocked on owner call 19 — wave 12 may be another **critic-only / stall**. That is allowed; two more stalls halt the run at `plateau: 3`.

## Wave 10 adjudications — Zoom row closes, Appearance ordinal moves, plateau resets

**One builder, three open-piece critics, zero smoothing edits.** Gate green (D7); **zero `SPEC BREAK`s**; critic family re-resolved live to `sonnet` -> `codex/gpt-5.6-sol`; `critic_degraded: false`. Final CSS bundle **`index-DOI17h8g.css`** (left wave 9's `index-d_l3D7IG`).

### 1. THE PLATEAU RESETS — ONE ORDINAL IMPROVED

| piece | before | after | builder? | change |
|---|---|---|---|---|
| AppearanceDock | BAR WINS | **YOURS WINS** | yes — Zoom control-row shell | BETTER |
| AgentsDock | BAR WINS | BAR WINS | no — collision A | SAME |
| DocksAsOne | BAR WINS | BAR WINS | no — critic-only | BETTER |
| CommandsDock | YOURS WINS | YOURS WINS | closed | — |
| WelcomeMinWindow | YOURS WINS | YOURS WINS | closed | — |
| IconHousing | YOURS WINS | YOURS WINS | closed | — |

On the preset's literal rule any improvement resets the counter, so **`plateau` 2 -> 0**. BETTER-inside-band on DocksAsOne does not itself move the ordinal, but Appearance's band change does. Halt still at `plateau: 3` — three clean waves away at the earliest. Owner call 19 still open, still default build-nothing on Agents shells. Owner call 20 still open, still not acted on.

### 2. THE ZOOM BUILD WAS THE ONLY CLEAN LEVER AND IT WORKED

Wave 9 named Zoom row integration as the non-colliding candidate. Wave 10 built exactly that:

- `appearance-field--control` on the Zoom section only (trailing present)
- resting `--tint-2` inset shell around the whole head (mark + ZOOM + compact stepper)
- stepper stays **88×26** inside a **235×38** head — not full-column (Z2 pin)
- Theme/Backdrop heads stay bare (Z1b)
- option resting shells + gap-4 held (B1/B2/B3)

Critic: *"The Zoom shell visibly closes the prior gap by binding its label and compact stepper into one deliberate row."* Verdict **YOURS WINS**. Piece closed.

### 3. APPEARANCE'S POST-CLOSE GAP IS NOT A REOPEN WARRANT

Critic asked for a selected-theme descriptor/preview so Theme rows "feel as authored as Backdrop." That is a **content** ask against Theme's deliberate swatch-only anatomy (theme states its case by being applied; Backdrop needs a sentence because the trade is invisible). **Do not reopen AppearanceDock by default to add Theme copy.** Recorded as post-close polish only.

### 4. AGENTS + DOCKS AS ONE STILL COLLISION A

Agents SAME by bytes. DocksAsOne BETTER inside BAR WINS from Appearance alone, and its gap still names Agents rhythm/hierarchy **without resting shells**. Owner call 19 still owns any shell fork. Default remains build nothing on Agents.

### 5. SMOOTHING PASS MOVED ZERO PIXELS

No edit. Piece list full at six of six. **NO_NEW_PIECE.** Smooth reason: control shell correctly marks the only header-with-trailing; Theme/Backdrop stay bare section heads over option shells — one system, two roles.

### 6. Verification that was run rather than trusted

- **Build before capture.** Bundle left `index-d_l3D7IG` for `index-DOI17h8g` before inspect ran.
- **Inspect PASS**, headroom 69.71 / claimed 70, overflow 0, 11/11 files under `.gauntlet/waves/docks-and-min-window/10/`.
- **SHA256: DIFF only `appearance-dock.png`.** SAME the other ten, including null controls `titlebar.png` and `sidebar.png`.
- **Drivers wave2..wave10 ALL GREEN.** Wave10: Z0/Z1/Z1b/Z2 + B1/B2/B3 holds + C1 fence, with live reconstructions that fail.
- **Gate:** typecheck clean; **96 files / 1406 passed + 44 skipped** (+1 attributed to `gui-gauntlet-wave10.mjs`); build clean, final hash `index-DOI17h8g`.
- **Critic routing re-resolved live** this wave: `sonnet` -> `codex/gpt-5.6-sol`. Eleven consecutive waves, each read fresh.

### 7. What wave 11 inherits

| piece | wave 11 posture | trap |
|---|---|---|
| AppearanceDock | **closed YOURS** | Theme descriptor is post-close polish — do not reopen by default; do not invert shells |
| AgentsDock / DocksAsOne | **still collision A** — only open pieces | row shell; owner call 19 — default remains build nothing |
| CommandsDock | closed YOURS | optional horizontal padding only if re-opened |
...
With plateau at 0 and only Agents/DocksAsOne still open — both blocked on owner call 19 — wave 11 may be **critic-only / stall** unless a non-shell Agents lever appears. That is allowed; three stalls halt the run.
...
**One builder, three open-piece critics, one comment-only smoothing pass.** Gate green (D7); **one `SPEC BREAK` filed and REFUSED against source**; critic family re-resolved live to `sonnet` -> `codex/gpt-5.6-sol`; `critic_degraded: false`. Final CSS bundle **`index-d_l3D7IG.css`** (left wave 7's `index-D6IIg5C6`).

### 1. THE PLATEAU RESUMES — NO ORDINAL IMPROVED

| piece | before | after | builder? | change |
|---|---|---|---|---|
| AppearanceDock | BAR WINS | BAR WINS | yes — gap 2 -> 4 | BETTER |
| AgentsDock | BAR WINS | BAR WINS | no — collision A | SAME |
| DocksAsOne | BAR WINS | BAR WINS | no — critic-only | BETTER |
| CommandsDock | YOURS WINS | YOURS WINS | closed | — |
| WelcomeMinWindow | YOURS WINS | YOURS WINS | closed | — |
...
On the preset's literal rule none improved ordinal, so **`plateau` 0 -> 1**. BETTER-inside-band does not reset. Halt still at `plateau: 3` — two clean waves away at the earliest. Owner call 20 still open, still not acted on; no regression this wave either way.

### 2. AGENTSDOCK'S `SPEC BREAK` IS REFUSED — "ROW SHELL" IS THE SHARED LAYOUT GROUP, NOT A RESTING INSET

The critic wrote: *"SPEC BREAK The Agents dock does not use the mandated row shell mirroring the Sessions rail."* DESIGN.md Layout says the Agents dock mirrors the Sessions rail with *"same 44px head, same row shell"*. That phrase has already been read in this run as **shared layout group** — padding, radius, gap, head height — not as a mandatory resting `--tint-2` inset on every bare entry.

Evidence already on the branch, not re-argued:

- Sessions and Agents paint **state fills only** at rest (transparent otherwise). Commands is the documented exception that took a resting inset because it is a **stateless** list and was otherwise invisible in a still frame (wave 6 adjudication 2; wave 7 held it).
- Appearance's resting shells are the same exception class for a preference picker, not a new mandate on Agents.
- Handing the SPEC BREAK to a builder would be collision A by another door: force Agents resting shells without resolving owner call 19, which is exactly the wave-6 mistake.

**Refused. Do not build Agents resting shells off this filing.** The underlying gap (scannable row anatomy, hierarchy, selection) can still be named without the false mandate — and still collides with DocksAsOne / owner call 19.

### 3. APPEARANCE'S NEW GAP INVERTS WAVE 7 — DO NOT BUILD IT AS WRITTEN

Wave 7's critic asked for resting shells on every option; wave 7 built them; wave 8 opened the gap to 4 so they stop reading as one corrugated strip. Wave 8's critic now asks to **remove shells from inactive Theme rows** and keep the shell only on the selected option.

That is not a small polish. It:

1. **Undoes the wave-7 anatomy** the gap was opened to protect.
2. **Re-opens the three-way (honestly two-way) resting-row split** DocksAsOne is already grading — Commands shells every row, Appearance would shell only selected, Agents stay open.
3. Collides with DocksAsOne's own wave-8 gap, which still wants Agents *onto* the shelled track (or shells off Commands and Appearance together).

**Do not hand "shells only on selected" to a wave-9 builder verbatim.** If a later critic keeps asking, the honest fork is owner-call-19-shaped: pick one resting-row grammar across the docks, not a per-dock oscillation. The gap-4 pin (`gui-gauntlet-wave8.mjs` B1) and the shell/mark holds (B2/B3) stay; the anatomy does not quietly reverse under a BETTER-band ask.

### 4. BOTH COLLISIONS HELD AT THE DEFAULT

**Collision A:** no builder on Agents, no builder on DocksAsOne. DocksAsOne's gap still names Agents resting shells after owner call 19. Default remains build nothing.

**Collision B:** IconHousing stays YOURS WINS with the capsule. No builder touched it. Do not remove the capsule.

### 5. SMOOTHING PASS MOVED ZERO PIXELS

Five comment-only edits realigning stale "every dock separates rows by 2px" notes to the live three-step ladder (Agents shared 2 / Appearance 4 / Commands 6). Touched `appearance.css` notes and one `AppearanceDock.tsx` hairline-deletion note. **No property values changed after the builder's gap 2 -> 4.** Bundle stays `index-d_l3D7IG`. **No new piece — list full at six of six.**

### 6. Verification that was run rather than trusted

- **Build before capture.** Bundle left `index-D6IIg5C6` for `index-d_l3D7IG` before inspect ran.
- **Inspect PASS**, headroom 69.71 / claimed 70, overflow 0, 11/11 files under `.gauntlet/waves/docks-and-min-window/8/`.
- **Drivers wave2..wave8 ALL GREEN.** Wave8: B1 gap 4 + live reconstruction at 2 differs + pairGaps ~4; B2 resting inset shells hold; B3 marks 12x12; C1 Commands 6 / Agents 2 fence.
- **Blast radius by SHA256:** DIFF only `appearance-dock.png`. SAME the other ten, including null controls `titlebar.png` and `sidebar.png`.
- **Gate:** typecheck clean; **96 files / 1406 passed + 43 skipped** (+1 attributed to `gui-gauntlet-wave8.mjs`); build clean, final hash `index-d_l3D7IG`.
- **Critic routing re-resolved live** this wave: `sonnet` -> `codex/gpt-5.6-sol`.

### 7. What wave 9 inherits

| piece | wave 9 posture | trap |
|---|---|---|
| AppearanceDock | **do not invert shells**; optional only if a non-inverting lever appears | "shells only on selected" is refused as written — adjudication 3 |
| AgentsDock / DocksAsOne | **still collision A** | row shell; owner call 19 — default remains build nothing; SPEC BREAK already refused |
| CommandsDock | closed YOURS | optional horizontal padding only if re-opened |
| WelcomeMinWindow | closed YOURS | preserve 24; do not tighten |
| IconHousing | closed YOURS | zoom-minus only if re-opened; **do not remove the capsule** |

With plateau at 1 and no clean ordinal lever unblocked by owner call 19, wave 9 may be a **critic-only / stall** wave unless a builder finds a non-colliding lever. That is allowed — plateau 1 -> 2 is honest progress toward halt.
...
- [wave 12] **CRITIC-ONLY STALL, ORDINALS HELD, PLATEAU 1 -> 2.** Zero builders,
  ...
  controls alive). Bundle still **`index-DOI17h8g.css`**. Critic re-resolved live:
  `sonnet` -> `codex/gpt-5.6-sol` (thirteenth consecutive, fresh). `plateau` **1 -> 2**.
  `wave` now **12 == max_waves** — next firing halts on budget backstop (or one more
  stall would hit plateau 3).
- [wave 12] **NO CLEAN PRIMARY BUILD — DEFAULT HELD.** Only open pair still
  collision A / owner call 19. Gaps restate Explore compact child (branch-rule
  height + desc-to-meta) and Agents top-level spacing / subordinate Explore type+indent
  without shells — no new non-shell lever. Appearance Theme descriptor still
  post-close polish, not a reopen.
- [wave 12] **BOTH COLLISIONS HELD.** Owner call 19 default. Capsule kept. Smooth:
  NO_NEW_PIECE, zero pixels. Drivers wave2..wave10 GREEN. Gate: typecheck clean,
  **96 files / 1406 passed + 44 skipped**. Nothing pushed. Next leg boots at
  wave=12 → stop on `wave >= max_waves` before building anything.
- [wave 11] **CRITIC-ONLY STALL, ORDINALS HELD, PLATEAU 0 -> 1.** Zero builders,
  two open-piece critics, zero smoothing edits. Agents + DocksAsOne stayed BAR
  WINS with change SAME. All eleven captures SHA256-identical to wave 10 (null
  controls alive). Bundle still **`index-DOI17h8g.css`**. Critic re-resolved live:
  `sonnet` -> `codex/gpt-5.6-sol` (twelfth consecutive, fresh). `plateau` **0 -> 1**.
  Halt two clean waves away at `plateau: 3`.
- [wave 11] **NO CLEAN PRIMARY BUILD — DEFAULT HELD.** Only open pair still
  collision A / owner call 19. Gaps restate left-edge / Explore hierarchy and
  Agents spacing without shells — no new non-shell lever. Appearance Theme
  descriptor still post-close polish, not a reopen.
- [wave 11] **BOTH COLLISIONS HELD.** Owner call 19 default. Capsule kept. Smooth:
  NO_NEW_PIECE, zero pixels. Drivers wave2..wave10 GREEN. Gate: typecheck clean,
  **96 files / 1406 passed + 44 skipped**. Nothing pushed. Wave 12 inherits only
  Agents/DocksAsOne open (collision A); may be critic-only/stall again.
- [wave 10] **ZOOM CONTROL-ROW SHELL, APPEARANCE BAR->YOURS, PLATEAU 2 -> 0.** One
  builder (AppearanceDock Zoom head `appearance-field--control` resting --tint-2
  shell), three open-piece critics, zero smoothing edits. Appearance **BAR WINS
  -> YOURS WINS** (change BETTER) and closed. Agents BAR WINS SAME (collision A).
  DocksAsOne BAR WINS BETTER inside band (no ordinal). SHA256 DIFF only
  `appearance-dock.png`; null controls alive. Bundle **`index-DOI17h8g.css`**.
  Critic re-resolved live: `sonnet` -> `codex/gpt-5.6-sol` (eleventh consecutive).
  `plateau` **2 -> 0**. Four of six now YOURS WINS.
- [wave 10] **ONLY CLEAN LEVER WORKED.** Zoom head shells mark+label+compact
  stepper as one composed row; Theme/Backdrop stay bare; option shells + gap-4
  held. Wave10 driver Z0/Z1/Z1b/Z2 + B1/B2/B3 + C1 GREEN with live reconstructions.
- [wave 10] **POST-CLOSE THEME DESCRIPTOR IS POLISH, NOT A REOPEN.** Critic asked
  for selected-theme descriptor/preview; Theme is swatch-only by design. Do not
  reopen Appearance by default.
- [wave 10] **BOTH COLLISIONS HELD.** Owner call 19 default. Capsule kept. Smooth:
  NO_NEW_PIECE, zero pixels. Drivers wave2..wave10 GREEN. Gate: typecheck clean,
  **96 files / 1406 passed + 44 skipped**. Nothing pushed. Wave 11 inherits only
  Agents/DocksAsOne open (collision A); may be critic-only/stall.
...
- [wave 8] **GAP OPENED, ORDINALS HELD, PLATEAU RESUMES.** One builder
  ...
  Appearance and DocksAsOne read BETTER inside the band; Agents SAME
  (pixel-identical). `plateau` **0 -> 1**. Final bundle **`index-d_l3D7IG.css`**.
  Critic re-resolved live: `sonnet` -> `codex/gpt-5.6-sol`.
- [wave 8] **AGENTSDOCK `SPEC BREAK` REFUSED.** Critic filed "mandated row shell
  mirroring Sessions"; DESIGN Layout + rails shared group mean padding/radius/gap,
  not resting inset. Building it would be collision A by another door.
- [wave 8] **APPEARANCE'S NEW GAP INVERTS WAVE 7 — DO NOT BUILD AS WRITTEN.**
  Critic asks shells only on selected Theme rows; that undoes resting-all-shells
  and re-opens the resting-row split DocksAsOne grades. Gap-4 pin and shell/mark
  holds stay; anatomy does not reverse under a BETTER-band ask.
- [wave 8] **BOTH COLLISIONS HELD.** Owner call 19 default. Capsule kept.
  Smooth: five comment-only edits, zero pixels, NO_NEW_PIECE. Blast by SHA256:
  DIFF only `appearance-dock.png`; null controls alive. Drivers wave2..wave8
  GREEN. Gate: typecheck clean, **96 files / 1406 passed + 43 skipped** (+1 =
  `gui-gauntlet-wave8.mjs`). Nothing pushed. Wave 9 may be critic-only/stall.
