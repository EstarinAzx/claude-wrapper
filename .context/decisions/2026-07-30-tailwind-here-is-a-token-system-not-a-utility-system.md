...
> **AMENDED 2026-08-04 by #94 ([[2026-08-04-the-font-shorthand-resets-the-line-box]]).** The clause above is **understated, and the deferral it justified is now paid off** (`e1a2c31`). `font` is a **shorthand**: it resets `line-height` as well as `font-family`. `rails.css` declares zero line-heights, `body` sets `1.6`, and a `<button>`'s UA `normal` does not inherit — so the naive join moved **all three** `.command-row-*` children (+5.6px / +4.8px / +5.6px) and a 60px row became 76px, not the one child predicted here. The two children that set their own `font-family` looked immune and were not. The fix restores `line-height: normal` on the button and pins `.command-row-desc` alone. **The error was not enumerating the shorthand** — a lesson that outlives this bug.
